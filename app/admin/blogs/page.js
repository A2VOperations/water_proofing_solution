  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <header className="mb-10 flex justify-between items-end">
        <div>
          <h1 className="text-4xl font-black uppercase tracking-tight text-[#111]">Manage Blogs</h1>
          <p className="text-gray-500 font-medium mt-2">Edit or remove your published articles.</p>
        </div>
        <Link href="/admin/addBlogs" className="bg-[#0088ff] text-white px-6 py-3 rounded-xl font-bold uppercase tracking-widest text-xs shadow-lg hover:bg-[#0070d6] transition-all">
          Add New Blog
        </Link>
      </header>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-[32px] border border-gray-100 p-20 text-center">
          <p className="text-gray-400 font-bold uppercase tracking-[0.2em]">No blog posts found</p>
          <Link href="/admin/addBlogs" className="text-[#0088ff] mt-4 inline-block font-black uppercase text-xs">Write your first post</Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {blogs.map((blog) => (
            ...
            >
              ...
              <div className="p-8">
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{new Date(blog.date).toLocaleDateString()}</span>
